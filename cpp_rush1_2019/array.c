...
typedef struct {
    ...
    ArrayClass  *_array;
    size_t      _idx;
}   ArrayIteratorClass;   /* Iterator struct */
...
static void ArrayIterator_setval(ArrayIteratorClass *this, ...)
{
    ...
    if (!this)
        raise("ArrayIteratorClass can't be NULL.");
    va_start(list, this);
    obj = va_new(this->_array->_type, &list);
    va_end(list);
    if (this->_idx > this->_array->_size)
        raise("Invalid index.");
...
static void Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    if (!this)
        raise("ArrayClass cannot be NULL.");
    ...
    for (size_t i = 0; i < this->_size; i += 1) {
        va_copy(cpy, *args);
        this->_tab[i] = va_new(this->_type, &cpy);
    }
}
...
static Object *Array_getitem(ArrayClass *this, ...)
{
    ...
    if (!this)
        raise("ArrayClass can't be NULL.");
    va_start(list, this);
...
static void Array_setitem(ArrayClass *this, ...)
{
    ...
    va_start(list, this);
    index = va_arg(list, size_t);
    Object *obj = va_new(this->_type, &list);
    va_end(list);
    if (index > this->_size)
        raise("Invalid index.");
