...
testWaterReclaimer()
{
    ...
    try {
        ...
        assert(std::strcmp(error.what(), "Not enough water to achieve the mission.") == 0);
    }
    assert(hasExcept);
    ...
    try {
        ...
        hasExcept = true;
        assert(error.getComponent() == "WaterReclaimer");
        assert(std::strcmp(error.what(), "Cannot generate water, reclaimer already full.") == 0);
    }
    assert(hasExcept);
    ...
    try {
        ...
        hasExcept = true;
        assert(std::strcmp(error.what(), "Cannot use water, not enough in the reclaimer.") == 0);
        assert(error.getComponent() == "WaterReclaimer");
    }
    assert(hasExcept);
...
testOxygenator()
{
    ...
    try {
        ...
        hasExcept = true;
        assert(error.getComponent() == "Oxygenator");
        assert(std::strcmp(error.what(), "Not enough oxygen to continue the mission.") == 0);
    }
    assert(hasExcept);
}
...
testEngine()
{
    ...
    try {
        ...
        assert(std::strcmp(error.what(), "Cannot reach destination (10, 10).") == 0);
    }
    assert(hasExcept);
